...
for pp = 1:size(sigsToMeasure,1)
    ...
    IntenMax(pp)= max(tmpData);
    IntenMin(pp)= min(tmpData);
    ...
    if ~isempty(dfPk2)
        ...
        sigAmpT(pp) = mean(tmpData(zx(startInd+1)-round(ampAvgWidth/2):zx(startInd+1)+round(ampAvgWidth/2)));
        sigAmp(pp) = sigAmpT(pp)- mean(tmpData(137:139));
...
%                     plot(df2Times,df2Data./max(abs(df2Data(:))),'LineWidth',2); 
                    ...
                    plot([tvals(1) tvals(end)],[sigAmpT(pp) sigAmpT(pp)],'--k','LineWidth',2);
...
T = num2cell([tsRiseCleaned; tsFallCleaned; sigWidthCleaned; ampsCleaned;IntenMax;  IntenMin]);
T = table([cellIDsCleaned; [{'Rise Time (sec)' 'Fall Time (sec)' 'Signal Width (sec)' 'Signal Amplitude' 'Max-Inten' 'Min-Inten'}' T]]);
